...
def plot_results(results, N_values, M_values, criteria):
    ...
    # First: varying N (samples), keeping M constant
    print("\nTime vs number of samples (N)\n")
    plt.figure(figsize=(18, 6*len(M_values))) 
    ...
    for i, M in enumerate(M_values):
        ...
        for input_type in ['discrete', 'real']:
            for output_type in ['discrete', 'real']:
                ...
                ax1.plot(N_values, train_times, marker='o',
                         label=f"{input_type} - {output_type}")
                ax2.plot(N_values, prediction_times, marker='o',
                         label=f"{input_type} - {output_type}")

        ax1.set_xlabel("Number of Samples (N)")
        ax1.set_ylabel("Training Time (s)")
        ax1.set_title(f"Train Time vs N (M={M})")
        ax1.grid(True)
        ...
        ax2.set_title(f"Test Time vs N (M={M})")
        ax2.grid(True)
    ...
    # Second: varying M (features), keeping N constant
    print("\nTime vs number of features (M)\n")
    plt.figure(figsize=(18, 6*len(N_values)))  # bigger figure
    ...
    for i, N in enumerate(N_values):
        ...
        for input_type in ['discrete', 'real']:
            for output_type in ['discrete', 'real']:
                ...
                ax1.plot(M_values, train_times, marker='o',
                         label=f"{input_type} - {output_type}")
                ax2.plot(M_values, prediction_times, marker='o',
                         label=f"{input_type} - {output_type}")

        ax1.set_xlabel("Number of Features (M)")
        ax1.set_ylabel("Training Time (s)")
        ax1.set_title(f"Train Time vs M (N={N})")
        ax1.grid(True)
        ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        ax1.tick_params(axis='x', rotation=30)
        ...
        ax2.set_ylabel("Prediction Time (s)")
        ax2.set_title(f"Test Time vs M (N={N})")
        ax2.grid(True)
        ax2.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        ax2.tick_params(axis='x', rotation=30)
